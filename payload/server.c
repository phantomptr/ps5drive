...
static int serve_web_index(server_ctx_t *ctx, int client_fd) {
    ...
    rc |= strbuf_append(&html,
                        ...
                        "<div id=\"uploadStatus\" class=\"mono\" style=\"font-size:.82rem;color:var(--muted);display:block;min-height:2.4em;line-height:1.25;white-space:normal;overflow-wrap:anywhere;word-break:break-word\">Idle</div></div>"
    ...
    rc |= strbuf_append(&html,
                        "<script>"
                        "const api='';"
                        "const state={path:'/',entries:[],selected:'',queueFiles:[],queueIsFolder:false,queueBytes:0,queueDirs:0,uploading:false,cancelUpload:false,currentXhr:null,selectedBusy:false,listOffset:0,listLimit:500,hasMore:false,listLoadingMore:false,scanActive:false,scanDone:0,scanTotal:0,scanToken:0,uploadUiTs:0,uploadUiPct:-1,uploadUiMsg:''};"
                        ...
                        "function addRelDirs(rel,dirSet){if(!rel||!dirSet)return 0;const parts=String(rel).split('/');if(parts.length<2)return 0;let cur='';let added=0;for(let i=0;i<parts.length-1;i++){const part=parts[i];if(!part)continue;cur=cur?cur+'/'+part:part;if(!dirSet[cur]){dirSet[cur]=1;added++;}}return added;}"
                        "function computeQueueStats(files){const dirSet={};let bytes=0;let dirs=0;let hasNested=false;for(const f of (files||[])){if(!f||typeof f.size!=='number')continue;bytes+=Number(f.size)||0;const rel=relPathOf(f);if(rel.indexOf('/')>=0)hasNested=true;dirs+=addRelDirs(rel,dirSet);}return {bytes,dirs,hasNested:hasNested||dirs>0};}"
                        ...
                        "function setUploadProgress(percent,msg,force){const bar=qs('uploadProgress');const status=qs('uploadStatus');const pct=Math.max(0,Math.min(100,Math.floor(Number(percent)||0)));const now=Date.now();if(bar&&(force||pct!==state.uploadUiPct))bar.value=pct;if(status&&msg!==undefined){const text=String(msg);if(force||now-state.uploadUiTs>=120||pct===0||pct===100||text.indexOf('failed')>=0||text.indexOf('Done')>=0||text.indexOf('stopped')>=0||text.indexOf('Scanning')===0){status.textContent=text;state.uploadUiTs=now;state.uploadUiMsg=text;}}state.uploadUiPct=pct;}"
                        "function setUploadUIState(){const uploading=!!state.uploading;const scanning=!!state.scanActive;const busy=uploading||scanning;const start=qs('uploadStartBtn');const stop=qs('uploadStopBtn');const pickFilesBtn=qs('pickFilesBtn');const pickFolderBtn=qs('pickFolderBtn');if(start)start.disabled=busy||!state.queueFiles.length;if(stop)stop.disabled=!uploading;if(pickFilesBtn)pickFilesBtn.disabled=busy;if(pickFolderBtn)pickFolderBtn.disabled=busy;}"
                        "function renderQueueInfo(){const info=qs('queueInfo');if(!info)return;if(state.scanActive){const total=Math.max(1,state.scanTotal);const pct=Math.min(100,Math.floor((state.scanDone/total)*100));info.textContent='Scanning '+state.scanDone+'/'+state.scanTotal+' ('+pct+'%) | files '+state.queueFiles.length+', folders '+state.queueDirs+', '+formatSize(state.queueBytes);setUploadUIState();return;}if(!state.queueFiles.length){info.textContent='No selection';setUploadUIState();return;}const fileWord=state.queueFiles.length===1?'file':'files';const folderPart=state.queueDirs>0?(', '+state.queueDirs+' folder'+(state.queueDirs===1?'':'s')):'';info.textContent='Selected '+state.queueFiles.length+' '+fileWord+folderPart+' ('+formatSize(state.queueBytes)+')';setUploadUIState();}"
                        "function setQueue(files,isFolder){if(state.uploading){log('Cannot change queue while upload is running');return;}const src=files?[...files]:[];const token=state.scanToken+1;state.scanToken=token;state.scanActive=true;state.scanDone=0;state.scanTotal=src.length;state.queueFiles=[];state.queueBytes=0;state.queueDirs=0;state.queueIsFolder=!!isFolder;state.uploadUiTs=0;state.uploadUiPct=-1;renderQueueInfo();setUploadUIState();if(!src.length){state.scanActive=false;renderQueueInfo();setUploadProgress(0,'Idle',true);return;}setUploadProgress(0,'Scanning selection 0/'+src.length+'...',true);let idx=0;const chunk=700;const dirSet={};let dirCount=0;const pump=()=>{if(token!==state.scanToken)return;const end=Math.min(src.length,idx+chunk);for(;idx<end;idx++){const f=src[idx];if(!f||typeof f.size!=='number')continue;const rel=relPathOf(f);if(!rel)continue;try{f.__rel_path=rel;}catch(e){}state.queueFiles.push(f);state.queueBytes+=Number(f.size)||0;dirCount+=addRelDirs(rel,dirSet);}state.scanDone=idx;state.queueDirs=dirCount;state.queueIsFolder=!!isFolder||dirCount>0;renderQueueInfo();setUploadProgress((state.scanDone/Math.max(state.scanTotal,1))*100,'Scanning selection '+state.scanDone+'/'+state.scanTotal+'...',false);if(idx<src.length){setTimeout(pump,0);return;}state.scanActive=false;state.scanDone=state.scanTotal;state.queueDirs=dirCount;state.queueIsFolder=!!isFolder||dirCount>0;renderQueueInfo();setUploadProgress(0,state.queueFiles.length?'Ready':'Idle',true);log('Scan done: '+state.queueFiles.length+' file(s), '+state.queueDirs+' folder(s), '+formatSize(state.queueBytes));};setTimeout(pump,0);}"
                        "function pickFiles(){if(state.uploading){log('Stop current upload first');return;}if(state.scanActive){log('Wait for scan to finish first');return;}qs('uploadFiles').click();}"
                        "function pickFolder(){if(state.uploading){log('Stop current upload first');return;}if(state.scanActive){log('Wait for scan to finish first');return;}qs('uploadFolder').click();}"
                        ...
                        "async function startUpload(){if(state.uploading){log('Upload already running');return;}if(state.scanActive){log('Wait for scan to finish before uploading');return;}if(!state.queueFiles.length){log('No files selected. Use Browse Files or Browse Folder.');return;}state.uploading=true;state.cancelUpload=false;setUploadUIState();const files=state.queueFiles.slice();const base=state.path;const totalBytes=Math.max(1,state.queueBytes||files.reduce((acc,f)=>acc+(Number(f.size)||0),0));let doneBytes=0;let uploaded=0;let speedBps=0;let canceled=false;let failed=false;const startTs=Date.now();let lastTs=startTs;let lastBytes=0;setUploadProgress(0,'Uploading 0/'+files.length,true);for(let i=0;i<files.length;i++){const f=files[i];if(state.cancelUpload){canceled=true;state.queueFiles=files.slice(i);break;}const rel=relPathOf(f);if(!rel)continue;const dst=join(base,rel);let allow=false;try{allow=await shouldUploadToPath(dst);}catch(err){if(state.cancelUpload||/canceled/i.test(String(err&&err.message||''))){canceled=true;state.queueFiles=files.slice(i);break;}failed=true;state.queueFiles=files.slice(i);setUploadProgress((doneBytes/totalBytes)*100,'Upload failed',true);log('Pre-check failed '+dst+': '+err.message);break;}if(!allow){doneBytes+=Number(f.size)||0;setUploadProgress((doneBytes/totalBytes)*100,'Skipped '+rel,false);continue;}try{await uploadOne(dst,f,(loaded,total)=>{const t=Number(total)||1;const now=Date.now();const doneNow=doneBytes+loaded;const dt=(now-lastTs)/1000;if(dt>=0.2){const inst=(doneNow-lastBytes)/Math.max(dt,0.001);speedBps=speedBps>0?(speedBps*0.7+inst*0.3):inst;lastTs=now;lastBytes=doneNow;}const avgBps=doneNow/Math.max((now-startTs)/1000,0.001);const showBps=speedBps>0?speedBps:avgBps;const remain=Math.max(0,totalBytes-doneNow);const eta=remain/Math.max(avgBps,1);const pct=(doneNow/Math.max(totalBytes,t))*100;setUploadProgress(pct,'Uploading '+(uploaded+1)+'/'+files.length+': '+rel+' | '+formatRate(showBps)+' | ETA '+formatEta(eta),false);});doneBytes+=Number(f.size)||0;uploaded+=1;const avgDoneBps=doneBytes/Math.max((Date.now()-startTs)/1000,0.001);setUploadProgress((doneBytes/totalBytes)*100,'Uploaded '+uploaded+'/'+files.length+' | '+formatRate(avgDoneBps),true);log('Uploaded '+dst);}catch(err){if(state.cancelUpload||/canceled/i.test(String(err&&err.message||''))){canceled=true;state.queueFiles=files.slice(i);break;}failed=true;state.queueFiles=files.slice(i);setUploadProgress((doneBytes/totalBytes)*100,'Upload failed',true);log('Upload failed '+dst+': '+err.message);break;}}if(!canceled&&!failed&&uploaded===files.length){state.queueFiles=[];state.queueIsFolder=false;state.queueBytes=0;state.queueDirs=0;const totalSec=Math.max((Date.now()-startTs)/1000,0.001);setUploadProgress(100,'Done '+uploaded+'/'+files.length+' | Avg '+formatRate(doneBytes/totalSec),true);}if(canceled){const remainCount=state.queueFiles.length;const pct=(doneBytes/totalBytes)*100;setUploadProgress(pct,'Upload stopped ('+remainCount+' remaining)',true);log('Upload stopped. Remaining '+remainCount+' file(s).');}state.uploading=false;state.cancelUpload=false;state.currentXhr=null;qs('uploadFiles').value='';qs('uploadFolder').value='';const stats=computeQueueStats(state.queueFiles);state.queueBytes=stats.bytes;state.queueDirs=stats.dirs;state.queueIsFolder=stats.hasNested;renderQueueInfo();setUploadUIState();if(uploaded>0)refreshList(false);}"
                        "async function onDrop(ev){ev.preventDefault();qs('dropZone').classList.remove('dragover');if(state.uploading){log('Stop current upload before adding more files');return;}if(state.scanActive){log('Wait for scan to finish first');return;}try{const files=await collectDropFiles(ev.dataTransfer);if(!files.length){log('No files detected from drop.');return;}setQueue(files,false);}catch(err){log('Drop parse failed: '+err.message);}}"
                        ...
                        "qs('dropZone').addEventListener('dragover',(ev)=>{ev.preventDefault();if(!state.uploading&&!state.scanActive)qs('dropZone').classList.add('dragover');});"
                        ...
                        "initTheme();setPath('/');updateSelectionUI();setSelectedOpProgress(0,'Selected action: idle');renderQueueInfo();setUploadUIState();setListUIState();refreshList(false);keepAlive();setInterval(keepAlive,15000);"
                        "</script></body></html>");
    if (rc != 0) {
        strbuf_free(&html);
        return send_json_error(client_fd, 500, "oom");
