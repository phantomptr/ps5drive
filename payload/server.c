...
static int send_auth_required(int fd) {
    ...
    if (send_fmt(fd,
                 ...
                 "WWW-Authenticate: Basic realm=\"" PS5DRIVE_BRAND "\"\r\n"
...
static int detect_game_title_id(const char *name, char *title_id, size_t title_id_len, char *platform, size_t platform_len) {
    ...
    if (!isdigit((unsigned char)upper[4]) ||
        !isdigit((unsigned char)upper[5]) ||
        !isdigit((unsigned char)upper[6]) ||
        !isdigit((unsigned char)upper[7]) ||
        !isdigit((unsigned char)upper[8])) {
        return 0;
    }

    const char *detected_platform = "PS4";
    if (strncmp(upper, "PPSA", 4) == 0 || strncmp(upper, "PPXA", 4) == 0) {
...
static int appmeta_build_full_path(const char *root_prefix, const char *abs_path, char *out_path, size_t out_len) {
    if (!abs_path || !*abs_path || abs_path[0] != '/' || !out_path || out_len == 0) return -1;
    if (root_prefix && root_prefix[0] != '\0' && strcmp(root_prefix, "/") != 0) {
        if (snprintf(out_path, out_len, "%s%s", root_prefix, abs_path) >= (int)out_len) return -1;
    } else {
        if (snprintf(out_path, out_len, "%s", abs_path) >= (int)out_len) return -1;
    }
    return 0;
}

static int appmeta_has_param_sfo_at_root(const char *title_id, const char *root_prefix) {
    ...
    for (size_t i = 0; i < sizeof(candidates) / sizeof(candidates[0]); ++i) {
        ...
        if (snprintf(abs_path, sizeof(abs_path), candidates[i], title_id) >= (int)sizeof(abs_path)) continue;
        if (appmeta_build_full_path(root_prefix, abs_path, full_path, sizeof(full_path)) != 0) continue;
        if (path_is_regular_file(full_path)) return 1;
    }
    return 0;
}

static int appmeta_find_cover_file_at_root(const char *title_id, const char *root_prefix, char *out_path, size_t out_len) {
    if (!title_id || !*title_id) return 0;
    const char *candidates[] = {
        "/user/appmeta/%s/icon0.png",
        "/user/appmeta/%s/icon0.jpg",
        "/user/appmeta/%s/icon0.jpeg",
        "/user/appmeta/%s/icon0.webp",
        "/user/appmeta/external/%s/icon0.png",
        "/user/appmeta/external/%s/icon0.jpg",
        "/user/appmeta/external/%s/icon0.jpeg",
        "/user/appmeta/external/%s/icon0.webp",
    };
    for (size_t i = 0; i < sizeof(candidates) / sizeof(candidates[0]); ++i) {
        char abs_path[PATH_MAX];
        char full_path[PATH_MAX];
        ...
        if (out_path && out_len > 0) snprintf(out_path, out_len, "%s", full_path);
        return 1;
    }
    return 0;
}
...
static int scan_games_recursive(const char *virt_dir, const char *full_dir, int depth, games_scan_ctx_t *scan) {
    ...
    while ((ent = readdir(dir)) != NULL) {
        ...
        if (!has_param_sfo && is_title_dir) has_param_sfo = appmeta_has_param_sfo_at_root(title_id, scan->root_prefix);
        int has_cover = find_game_cover_file(child_full, NULL, 0);
        if (!has_cover && is_title_dir) has_cover = appmeta_find_cover_file_at_root(title_id, scan->root_prefix, NULL, 0);
        if (!is_sce_sys && (is_title_dir || has_param_sfo)) {
            ...
            /* Installed title roots can be huge; avoid descending once identified. */
            continue;
        }
        ...
        *next = '\0';
        ...
        if (colon) {
            *colon = '\0';
            ...
            } else if (strcasecmp(name, PS5DRIVE_RESET_USER_HEADER) == 0 ||
                       strcasecmp(name, "X-PS5Drive-Reset-User") == 0) {
                snprintf(req->reset_user, sizeof(req->reset_user), "%s", value);
            } else if (strcasecmp(name, PS5DRIVE_RESET_PASS_HEADER) == 0 ||
                       strcasecmp(name, "X-PS5Drive-Reset-Pass") == 0) {
...
static int handle_api_games_scan(server_ctx_t *ctx, int client_fd, const http_request_t *req) {
    ...
    scan.truncated = 0;
    snprintf(scan.root_prefix, sizeof(scan.root_prefix), "%s", ctx->root_abs);
...
static int handle_api_storage_list(server_ctx_t *ctx, int client_fd) {
    ...
    if (rc == 0) {
        char mnt_clean[PATH_MAX];
        char mnt_full[PATH_MAX];
        struct stat mnt_st;
        if (sanitize_virtual_path("/mnt", mnt_clean, sizeof(mnt_clean)) == 0 &&
            build_full_path(ctx, mnt_clean, mnt_full, sizeof(mnt_full)) == 0 &&
            stat(mnt_full, &mnt_st) == 0 && S_ISDIR(mnt_st.st_mode)) {
            DIR *mnt_dir = opendir(mnt_full);
            if (mnt_dir) {
                struct dirent *ent;
                while ((ent = readdir(mnt_dir)) != NULL) {
                    if (strcmp(ent->d_name, ".") == 0 || strcmp(ent->d_name, "..") == 0) continue;
                    if (strncmp(ent->d_name, "usb0", 4) != 0 &&
                        strncmp(ent->d_name, "ext0", 4) != 0 &&
                        strncmp(ent->d_name, "ext1", 4) != 0) {
                        continue;
                    }

                    char virt[PATH_MAX];
                    if (snprintf(virt, sizeof(virt), "/mnt/%s", ent->d_name) >= (int)sizeof(virt)) continue;
                    ...
                    int duplicate = 0;
                    for (size_t i = 0; i < sizeof(candidates) / sizeof(candidates[0]); ++i) {
                        if (strcmp(clean, candidates[i]) == 0) {
                            duplicate = 1;
                            break;
                        }
                    }
...
static int handle_api_games_cover(server_ctx_t *ctx, int client_fd, const http_request_t *req) {
    ...
    if (!find_game_cover_file(full, cover_path, sizeof(cover_path))) {
        ...
        title_id[0] = '\0';
        platform[0] = '\0';
        const char *base = basename_ptr(virt);
        if (detect_game_title_id(base, title_id, sizeof(title_id), platform, sizeof(platform)) == 0 ||
            !appmeta_find_cover_file_at_root(title_id, ctx->root_abs, cover_path, sizeof(cover_path))) {
...
static int serve_web_index(server_ctx_t *ctx, int client_fd) {
    ...
    int rc = 0;
    const char *web_title = PS5DRIVE_BRAND;
    rc |= strbuf_append(&html, "<!doctype html><html><head><meta charset=\"utf-8\"><title>");
    rc |= strbuf_append(&html, web_title);
    rc |= strbuf_append(&html, "</title>"
                              ...
                              "--btn-coffee-bg:#f4ba62;--btn-coffee-border:#cf8e2b;--btn-coffee-hover:#e5a64b;--btn-coffee-ink:#2b1905;"
                              "--btn-discord-bg:#6a75ff;--btn-discord-border:#4e58d8;--btn-discord-hover:#5a66f2;--btn-discord-ink:#f7f8ff;"
                              "--btn-github-bg:#111827;--btn-github-border:#0b1220;--btn-github-hover:#1f2937;--btn-github-ink:#f8fafc;"
                              ...
                              "--btn-coffee-bg:#c9892f;--btn-coffee-border:#9f6318;--btn-coffee-hover:#b77728;--btn-coffee-ink:#fff4de;"
                              "--btn-discord-bg:#7d87ff;--btn-discord-border:#6671e2;--btn-discord-hover:#707af0;--btn-discord-ink:#ffffff;"
                              "--btn-github-bg:#0b1220;--btn-github-border:#334155;--btn-github-hover:#1e293b;--btn-github-ink:#f8fafc;"
                              "--btn-danger-bg:#4b2631;--btn-danger-border:#8e3c50;--card-shadow:rgba(0,0,0,.5)}"
                              "*{box-sizing:border-box}html,body{height:100%;max-width:100%}body{margin:0;font-family:ui-rounded,'Trebuchet MS','Segoe UI',sans-serif;color:var(--sloth-ink);"
                              "background:linear-gradient(180deg,var(--sloth-bg),var(--sloth-bg2));min-height:100dvh;overflow-x:hidden;overflow-y:auto;overscroll-behavior-y:contain}"
                              ".shell,.sloth-shell{width:min(100%,100vw);max-width:none;min-height:100dvh;margin:0;padding:clamp(.55rem,1.4vw,1rem);display:grid;gap:clamp(.55rem,1.2vw,1rem)}"
                              ...
                              ".lang-chip,.sloth-lang-chip{font-size:.8rem;background:var(--chip-bg);border:1px solid var(--sloth-line);border-radius:999px;padding:.35rem .75rem;min-height:34px;color:var(--sloth-ink)}"
                              ...
                              "#filesPane{display:flex;flex-direction:column;flex:1 1 auto;min-height:0}"
                              "#gamesPane{flex:1 1 auto;min-height:0}"
                              "#filesPane .tablewrap,#filesPane .sloth-tablewrap{flex:1 1 auto;min-height:0;max-height:none;overflow:auto}"
                              ...
                              ".btn-coffee,.sloth-btn-coffee{background:linear-gradient(135deg,var(--btn-coffee-bg),var(--btn-coffee-hover));border-color:var(--btn-coffee-border);color:var(--btn-coffee-ink);box-shadow:inset 0 1px 0 rgba(255,255,255,.28)}"
                              ".btn-coffee:hover,.sloth-btn-coffee:hover{background:linear-gradient(135deg,var(--btn-coffee-hover),var(--btn-coffee-bg))}"
                              ".btn-discord,.sloth-btn-discord{background:linear-gradient(135deg,var(--btn-discord-bg),var(--btn-discord-hover));border-color:var(--btn-discord-border);color:var(--btn-discord-ink);box-shadow:inset 0 1px 0 rgba(255,255,255,.2)}"
                              ".btn-discord:hover,.sloth-btn-discord:hover{background:linear-gradient(135deg,var(--btn-discord-hover),var(--btn-discord-bg))}"
                              ".btn-github,.sloth-btn-github{background:linear-gradient(135deg,var(--btn-github-bg),var(--btn-github-hover));border-color:var(--btn-github-border);color:var(--btn-github-ink);box-shadow:inset 0 1px 0 rgba(255,255,255,.12)}"
                              ".btn-github:hover,.sloth-btn-github:hover{background:linear-gradient(135deg,var(--btn-github-hover),var(--btn-github-bg))}"
                              ...
                              "@media (max-height:860px){.grid,.sloth-grid{height:auto}.tablewrap,.sloth-tablewrap{max-height:48vh}.log,.sloth-log{max-height:32vh}}"
    ...
    rc |= strbuf_appendf(&html,
                         ...
                         "<div class=\"top-left sloth-top-left\"><div class=\"brand-wrap sloth-brand-wrap\"><img class=\"brand-logo sloth-brand-logo logo-light\" src=\"/assets/logo-light.svg\" alt=\"" PS5DRIVE_BRAND " logo\">"
                         "<img class=\"brand-logo sloth-brand-logo logo-dark\" src=\"/assets/logo-dark.svg\" alt=\"" PS5DRIVE_BRAND " logo\"></div>"
                         "<div id=\"versionText\" class=\"brand-sub sloth-brand-sub mono\">v%s</div>"
                         "<div id=\"createdByText\" class=\"meta sloth-meta\">Created by <a href=\"https://github.com/phantomptr\" target=\"_blank\" rel=\"noopener noreferrer\">PhantomPtr</a> · "
                         "<a href=\"https://ko-fi.com/B0B81S0WUA\" target=\"_blank\" rel=\"noopener noreferrer\"><span id=\"coffeeText\">Buy Me a Coffee</span></a> · "
                         "<a id=\"discordText\" href=\"https://discord.com/invite/fzK3xddtrM\" target=\"_blank\" rel=\"noopener noreferrer\">Discord</a> · "
                         "<a id=\"githubText\" href=\"https://github.com/phantomptr/ps5drive\" target=\"_blank\" rel=\"noopener noreferrer\">GitHub</a></div></div>"
                         "<div class=\"top-right sloth-top-right\"><div class=\"chips sloth-chips\">"
                         "<div id=\"apiPortChip\" class=\"chip sloth-chip mono\">API Port: %d</div><div id=\"debugPortChip\" class=\"chip sloth-chip mono\">Debug Port: %d</div><div id=\"modeChip\" class=\"chip sloth-chip mono %s\">Mode: %s</div><button id=\"themeToggleBtn\" class=\"btn-ghost sloth-btn-ghost theme-toggle sloth-theme-toggle\" aria-label=\"Toggle theme\" title=\"Toggle theme\">"
                         ...
                         "</button><select id=\"langSelect\" class=\"mono lang-chip sloth-lang-chip\" style=\"min-width:168px;max-width:220px\"><option value=\"en\">English</option><option value=\"zh-cn\">简体中文</option><option value=\"zh-tw\">繁體中文</option><option value=\"hi\">हिन्दी</option><option value=\"es\">Español</option><option value=\"ar\">العربية</option><option value=\"bn\">বাংলা</option><option value=\"pt-br\">Português (Brasil)</option><option value=\"ru\">Русский</option><option value=\"ja\">日本語</option><option value=\"de\">Deutsch</option><option value=\"fr\">Français</option><option value=\"ko\">한국어</option><option value=\"tr\">Türkçe</option><option value=\"vi\">Tiếng Việt</option><option value=\"id\">Bahasa Indonesia</option><option value=\"it\">Italiano</option><option value=\"th\">ไทย</option></select></div>"
                         "<div class=\"header-actions sloth-header-actions\">"
                         ...
                         "<button id=\"stopBtn\" class=\"btn-danger sloth-btn-danger\">Stop " PS5DRIVE_BRAND "</button></div></div></div>",
    ...
    rc |= strbuf_append(&html,
                        ...
                        "<button id=\"overwriteAllBtn\" class=\"btn-danger sloth-btn-danger\">Overwrite: All</button></div>"
                        ...
                        "<div class=\"row sloth-row\"><button id=\"selectPathBtn\">Select Current Path</button><button id=\"goSelectedBtn\">Go To</button><button id=\"downloadSelectedBtn\">Download</button>"
                        ...
                        "<div class=\"row sloth-row\"><input id=\"moveInput\" class=\"mono\" placeholder=\"example: " PS5DRIVE_STATE_DIR "\">"
                        ...
                        "<div id=\"moveTip\" class=\"meta sloth-meta\">Move/Copy tip: use absolute path (example: <span class=\"mono\">" PS5DRIVE_STATE_DIR "</span>). If destination is an existing folder, item name is kept.</div>"
                        "</div>"
                        "</div>"
                        ...
                        "<button id=\"refreshBtn\">Refresh</button><button id=\"backBtn\">Back</button><button id=\"upBtn\">Up</button><button id=\"rootBtn\">Root</button><button id=\"loadMoreBtn\" class=\"btn-ghost sloth-btn-ghost\" disabled>Load More</button></div>"
                        ...
                        "<div class=\"row sloth-row\"><input id=\"gamesPathInput\" class=\"mono\" placeholder=\"Optional extra suffix/paths (comma/newline, e.g. ps5/games or /data/custom)\">"
                        ...
                        "<div class=\"progress sloth-progress\" style=\"margin-top:.35rem\"><progress id=\"gamesScanProgress\" value=\"0\" max=\"100\"></progress>"
                        "<div id=\"gamesScanProgressText\" class=\"mono\" style=\"font-size:.8rem;color:var(--muted)\"></div></div>"
    ...
    rc |= strbuf_append(&html,
                        "<script>"
                        "const api='';"
                        "const BRAND_NAME='" PS5DRIVE_BRAND "';"
                        "const BRAND_ID='" PS5DRIVE_BRAND_LOWER "';"
                        "const DEFAULT_STATE_DIR='" PS5DRIVE_STATE_DIR "';"
                        "const RESET_USER_HEADER='" PS5DRIVE_RESET_USER_HEADER "';"
                        ...
                        "function initTheme(){let saved='';try{saved=localStorage.getItem(THEME_KEY)||'';}catch(e){}applyTheme(saved);}"
                        ...
                        "async function resetConfig(){if(!confirm('Reset config.ini to default and disable secure mode?'))return;const headers={};if(state.securityMode==='secure'){const user=prompt('Enter username to confirm reset:','');if(user===null)return;const pass=prompt('Enter password to confirm reset:','');if(pass===null)return;headers[RESET_USER_HEADER]=user;headers[RESET_PASS_HEADER]=pass;}try{const r=await apiJson('/api/config/reset',{method:'POST',headers:headers});updateSecurityMode((r&&r.security_mode)||'unsecure');log('Config reset to defaults.');}catch(err){log('Config reset failed: '+err.message);}}"
                        "function syncBackButton(){const btn=qs('backBtn');if(btn)btn.disabled=state.pathHistoryIndex<=0;}"
                        "function setPath(p,trackHistory,replaceHistory){const next=normPath(p||'/');state.path=next;qs('pathInput').value=state.path;qs('pathLabel').textContent=state.path;const hint=qs('uploadPathHint');if(hint)hint.textContent='Upload target: '+state.path;const track=trackHistory!==false;if(track){if(state.pathHistoryIndex<0||!state.pathHistory.length){state.pathHistory=[next];state.pathHistoryIndex=0;}else if(replaceHistory===true){state.pathHistory[state.pathHistoryIndex]=next;}else if(state.pathHistory[state.pathHistoryIndex]!==next){state.pathHistory=state.pathHistory.slice(0,state.pathHistoryIndex+1);state.pathHistory.push(next);state.pathHistoryIndex=state.pathHistory.length-1;}}else if(replaceHistory===true&&state.pathHistoryIndex>=0&&state.pathHistoryIndex<state.pathHistory.length){state.pathHistory[state.pathHistoryIndex]=next;}syncBackButton();}"
                        "function goBackPath(){if(state.pathHistoryIndex<=0)return;state.pathHistoryIndex-=1;const target=state.pathHistory[state.pathHistoryIndex]||'/';setPath(target,false);refreshList(false,false);}"
                        ...
                        "function uiTr(key,fallback){try{if(typeof window!=='undefined'&&typeof window.driveTr==='function'){const v=window.driveTr(key);if(v!==undefined&&v!==null&&v!=='')return v;}}catch(e){}return fallback||key;}"
                        ...
                        "function selectCurrentPath(){selectPath(state.path);}"
                        ...
                        "async function refreshList(append,trackHistory){const isAppend=!!append;const track=trackHistory!==false;try{let reqPath=normPath(qs('pathInput').value||state.path);if(isAppend){if(!state.hasMore||state.listLoadingMore)return;reqPath=state.path;state.listLoadingMore=true;setListUIState();}else{state.listOffset=0;state.hasMore=false;state.listLoadingMore=false;setPath(reqPath,track);}const off=isAppend?state.listOffset:0;const basePath=state.path;const d=await apiJson('/api/list?path='+qp(reqPath)+'&offset='+String(off)+'&limit='+String(state.listLimit));const realPath=normPath(d.path||reqPath);if(realPath!==basePath){if(isAppend){setPath(realPath,false,true);}else{setPath(realPath,track,true);}}const incoming=[...(d.entries||[])];incoming.sort((a,b)=>{if(!!a.is_dir!==!!b.is_dir)return a.is_dir?-1:1;return String(a.name).localeCompare(String(b.name));});if(isAppend&&realPath===basePath){state.entries=state.entries.concat(incoming);}else{state.entries=incoming;state.selected='';updateSelectionUI();}const next=Number(d.next_offset);state.listOffset=(Number.isFinite(next)&&next>=0)?next:(off+incoming.length);state.hasMore=!!d.has_more;renderRows();}catch(err){log('List failed: '+err.message);}finally{if(state.listLoadingMore)state.listLoadingMore=false;setListUIState();}}"
                        ...
                        "qs('refreshBtn').addEventListener('click',()=>refreshList(false));qs('backBtn').addEventListener('click',goBackPath);qs('upBtn').addEventListener('click',goUp);qs('rootBtn').addEventListener('click',()=>{setPath('/');refreshList(false);});qs('loadMoreBtn').addEventListener('click',loadMore);"
                        "qs('mkdirBtn').addEventListener('click',mkdirPath);qs('deleteSelectedBtn').addEventListener('click',()=>deletePath(state.selected));qs('renameBtn').addEventListener('click',renameSelected);qs('moveBtn').addEventListener('click',moveSelected);qs('copyBtn').addEventListener('click',copySelected);qs('chmodBtn').addEventListener('click',chmodSelected);qs('selectPathBtn').addEventListener('click',selectCurrentPath);qs('goSelectedBtn').addEventListener('click',goSelected);qs('downloadSelectedBtn').addEventListener('click',downloadSelected);"
    ...
    rc |= strbuf_append(&html,
                        ...
                        "let gamesStorageUserSet=false;"
                        "let gamesScanBusy=false;"
#if defined(PS5DRIVE_PS4_BUILD)
                        "const isPs4Build=true;"
                        "const gamesPresetScanPaths=['app'];"
                        "const gamesScanMaxDepth=2;"
                        "const gamesScanMaxDirs=3000;"
#else
                        "const isPs4Build=false;"
                        "const gamesPresetScanPaths=['etaHEN/games','etaHEN/homebrew','games','homebrew'];"
                        "const gamesScanMaxDepth=4;"
                        "const gamesScanMaxDirs=2500;"
#endif
                        "function q(id){return document.getElementById(id);}"
                        "function rewriteBrand(v){let s=String(v===undefined||v===null?'':v);s=s.split('PS5Drive').join(BRAND_NAME);s=s.split('/data/ps5drive').join(DEFAULT_STATE_DIR);s=s.split('ps5drive').join(BRAND_ID);return s;}"
                        "function tr(key){const base=I18N.en||{};const dict=I18N[currentLang]||base;return rewriteBrand(dict[key]||base[key]||key);}"
                        "window.driveTr=tr;window.ps5driveTr=tr;"
                        ...
                        "function setLang(lang){currentLang=chooseLang(lang);try{localStorage.setItem(LANG_KEY,currentLang);}catch(e){}document.documentElement.lang=currentLang;const sel=q('langSelect');if(sel)sel.value=currentLang;applyStaticText();applyDynamicText();if(typeof renderRows==='function')renderRows();if(typeof setListUIState==='function')setListUIState();if(typeof updateSelectionUI==='function')updateSelectionUI();renderGamesStorageFilters();renderGames();}"
                        "function applyStaticText(){setText('downloadConfigBtn','download_config');setText('uploadConfigBtn','upload_config');setText('resetConfigBtn','reset_config');setText('stopBtn','stop_payload');setText('coffeeText','buy_me_coffee');setText('uploadTitle','upload');setText('pickFilesBtn','browse_files');setText('pickFolderBtn','browse_folder');setText('dropZone','drop_hint');setText('uploadStartBtn','upload');setText('resumeUploadBtn','resume');setText('uploadStopBtn','stop');setText('createFolderTitle','create_folder');setText('mkdirBtn','create');setText('selectedTitle','selected_item');setText('goSelectedBtn','go_to');setText('downloadSelectedBtn','download');setText('deleteSelectedBtn','delete');setText('renameBtn','rename');setText('moveBtn','move_to');setText('copyBtn','copy_to');setText('chmodBtn','chmod');setText('tabFilesBtn','files');setText('tabGamesBtn','games');setText('refreshBtn','refresh');setText('backBtn','back');setText('upBtn','up');setText('rootBtn','root');setText('thName','name');setText('thSize','size');setText('thMTime','mtime');setText('thAction','action');setText('logTitle','activity_log');setText('clearLogBtn','clear');setText('gamesTitle','games_scanner');const gamesPreset=q('gamesPresetInfo');if(gamesPreset){gamesPreset.textContent=isPs4Build?'Preset scan target: /user/app':'Preset scan targets: /data and /mnt/{usb,ext} + etaHEN/games, etaHEN/homebrew, games, homebrew, and installed /user/app + /mnt/ext*/user/app';}setText('gamesScanBtn','scan');setText('gamesThCover','cover');setText('gamesThMeta','metadata');setText('gamesThPath','path');setText('gamesThAction','action');const uploadNote=q('uploadNote');if(uploadNote)uploadNote.textContent=tr('upload_note');const moveTip=q('moveTip');if(moveTip)moveTip.textContent=tr('move_tip');const mk=q('mkdirInput');if(mk)mk.placeholder='new-folder';const rn=q('renameInput');if(rn)rn.placeholder='new-name (same folder)';const mv=q('moveInput');if(mv)mv.placeholder='example: '+DEFAULT_STATE_DIR;}"
                        ...
                        "function normStorageRoots(list){const out=[];for(const raw of (list||[])){if(typeof raw!=='string')continue;const n=(typeof normPath==='function')?normPath(raw):String(raw||'/');if(out.indexOf(n)<0)out.push(n);}out.sort((a,b)=>{if(isPs4Build){if(a==='/user'&&b!=='/user')return -1;if(b==='/user'&&a!=='/user')return 1;}else{if(a==='/data'&&b!=='/data')return -1;if(b==='/data'&&a!=='/data')return 1;if(a==='/user'&&b!=='/user')return -1;if(b==='/user'&&a!=='/user')return 1;}return String(a).localeCompare(String(b));});return out;}"
                        ...
                        "function isAllowedGamesRoot(pathValue){const n=(typeof normPath==='function')?normPath(pathValue||'/'):String(pathValue||'/');if(isPs4Build)return (n==='/user'||n.startsWith('/user/')||n.startsWith('/mnt/ext'));if(n==='/data'||n==='/user'||n.startsWith('/user/'))return true;return n.startsWith('/mnt/usb0')||n.startsWith('/mnt/ext0')||n.startsWith('/mnt/ext1');}"
                        "function inferStorageRoot(pathValue){const p=(typeof normPath==='function')?normPath(pathValue||'/'):String(pathValue||'/');let best='';for(const raw of gamesStorageRoots){const root=(typeof normPath==='function')?normPath(raw||'/'):String(raw||'/');if(p===root||p.startsWith(root+'/')){if(root.length>best.length)best=root;}}if(best)return best;if(isPs4Build)return '/user';if(p==='/data'||p.startsWith('/data/'))return '/data';if(p==='/user'||p.startsWith('/user/'))return '/user';if(p.startsWith('/mnt/')){const parts=p.split('/').filter(Boolean);if(parts.length>=2)return '/'+parts[0]+'/'+parts[1];}return '/data';}"
                        "function syncVisibleStorage(){gamesStorageRoots=normStorageRoots(gamesStorageRoots);if(!gamesStorageRoots.length)gamesStorageRoots=[isPs4Build?'/user':'/data'];const keep=[];for(const root of gamesVisibleStorage){if(gamesStorageRoots.indexOf(root)>=0&&keep.indexOf(root)<0)keep.push(root);}if(!gamesStorageUserSet){gamesVisibleStorage=gamesStorageRoots.slice();return;}gamesVisibleStorage=keep.length?keep:gamesStorageRoots.slice();}"
                        "function renderGamesStorageFilters(){const box=q('gamesStorageFilters');if(!box)return;syncVisibleStorage();box.innerHTML='';if(!gamesStorageRoots.length||gamesStorageRoots.length<=1)return;const label=document.createElement('span');label.className='meta sloth-meta';label.style.alignSelf='center';label.textContent=tr('storage_filter')+':';box.appendChild(label);const allBtn=document.createElement('button');const allSelected=gamesVisibleStorage.length===gamesStorageRoots.length;allBtn.className=(allSelected?'btn-main sloth-btn-main':'btn-ghost sloth-btn-ghost')+' mini';allBtn.textContent=tr('all');allBtn.onclick=()=>{gamesStorageUserSet=true;gamesVisibleStorage=gamesStorageRoots.slice();renderGamesStorageFilters();renderGames();};box.appendChild(allBtn);for(const root of gamesStorageRoots){const count=gamesItems.filter((it)=>(it.storage_path||inferStorageRoot(it.path||'/'))===root).length;const active=gamesVisibleStorage.indexOf(root)>=0;const btn=document.createElement('button');btn.className=(active?'btn-main sloth-btn-main':'btn-ghost sloth-btn-ghost')+' mini';btn.textContent=root+(count>0?' ('+count+')':'');btn.onclick=()=>{gamesStorageUserSet=true;if(active){if(gamesVisibleStorage.length<=1)return;gamesVisibleStorage=gamesVisibleStorage.filter((v)=>v!==root);}else{gamesVisibleStorage.push(root);}syncVisibleStorage();renderGamesStorageFilters();renderGames();};box.appendChild(btn);}}"
                        "function mountRootFromPath(pathValue){const n=(typeof normPath==='function')?normPath(pathValue||'/'):String(pathValue||'/');if(!n.startsWith('/mnt/'))return '';const parts=n.split('/').filter(Boolean);if(parts.length<2)return '';return '/'+parts[0]+'/'+parts[1];}"
                        "function rootSupportsPresetSuffixes(root){if(isPs4Build)return true;const n=(typeof normPath==='function')?normPath(root||'/'):String(root||'/');if(n==='/user'||n.startsWith('/user/'))return false;return true;}"
                        "function buildInstalledTitleTargets(){const out=[];const seen={};const add=(path)=>{const n=(typeof normPath==='function')?normPath(path||'/'):String(path||'/');if(!n||seen[n])return;seen[n]=1;out.push(n);};add('/user/app');for(const root of gamesStorageRoots){const mount=mountRootFromPath(root);if(!mount)continue;add(mount+'/user/app');}return out;}"
                        "function parseGamesExtraPaths(raw){const suffixes=[];const absolute=[];const seenSuffix={};const seenAbsolute={};const parts=String(raw||'').split(/[\\n,]+/);for(const part of parts){const token=String(part||'').trim();if(!token)continue;if(token[0]==='/'){const abs=(typeof normPath==='function')?normPath(token):token;if(!seenAbsolute[abs]){seenAbsolute[abs]=1;absolute.push(abs);}continue;}const suffix=String(token).replace(/\\\\/g,'/').replace(/^\\/+/, '').replace(/\\/+$/, '').replace(/\\/+/g, '/');if(!suffix)continue;if(!seenSuffix[suffix]){seenSuffix[suffix]=1;suffixes.push(suffix);}}return {suffixes:suffixes,absolute:absolute};}"
                        "function setGamesScanProgress(done,total,currentPath){const bar=q('gamesScanProgress');const text=q('gamesScanProgressText');const t=Math.max(0,Number(total)||0);const d=Math.max(0,Math.min(t,Number(done)||0));const pct=t>0?Math.floor((d*100)/t):0;if(bar)bar.value=pct;if(!text)return;if(t<=0){text.textContent='';return;}const prefix=d>=t?tr('scan'):tr('scanning');text.textContent=prefix+' '+d+'/'+t+(currentPath?(' | '+currentPath):'');}"
                        "async function fetchStorageRoots(){try{const data=await apiJson('/api/storage/list');const rows=Array.isArray(data&&data.storage)?data.storage:[];const roots=[];const addRoot=(value)=>{const n=(typeof normPath==='function')?normPath(value||'/'):String(value||'/');if(!isAllowedGamesRoot(n))return;if(roots.indexOf(n)<0)roots.push(n);};for(const row of rows){const raw=(row&&typeof row==='object')?row.path:row;const txt=String(raw||'').trim();if(!txt)continue;addRoot((typeof normPath==='function')?normPath(txt):txt);}if(isPs4Build){if(roots.indexOf('/user')<0)roots.unshift('/user');return normStorageRoots(roots);}if(roots.indexOf('/data')<0)roots.unshift('/data');if(roots.indexOf('/user')<0)roots.push('/user');return normStorageRoots(roots);}catch(err){if(typeof log==='function')log('Storage list failed: '+(err&&err.message?err.message:String(err)));return isPs4Build?['/user']:['/data','/user'];}}"
                        "async function scanGamesPath(pathValue){const url='/api/games/scan?path='+(typeof qp==='function'?qp(pathValue):encodeURIComponent(pathValue))+'&max_depth='+String(gamesScanMaxDepth)+'&max_dirs='+String(gamesScanMaxDirs);const resp=await fetch(api+url);const text=await resp.text();let data={};try{data=text?JSON.parse(text):{};}catch(e){data={};}if(!resp.ok){const msg=(data&&data.error)?data.error:(text||('HTTP '+resp.status));return {ok:false,status:resp.status,error:msg,data:data};}return {ok:true,status:resp.status,data:data};}"
                        ...
                        "async function scanGames(){const input=q('gamesPathInput');const status=q('gamesStatus');const scanBtn=q('gamesScanBtn');if(gamesScanBusy)return;const rawExtra=input?String(input.value||''):'';const extras=isPs4Build?{suffixes:[],absolute:[]}:parseGamesExtraPaths(rawExtra);if(input){if(isPs4Build){input.value='';}else{const normalized=[];for(const s of extras.suffixes)normalized.push(s);for(const a of extras.absolute)normalized.push(a);input.value=normalized.join(', ');}}gamesScanBusy=true;if(scanBtn)scanBtn.disabled=true;if(status)status.textContent=tr('scanning')+'...';setGamesScanProgress(0,1,'');try{gamesStorageRoots=await fetchStorageRoots();syncVisibleStorage();const suffixes=gamesPresetScanPaths.concat(extras.suffixes||[]);const targets=[];for(const root of gamesStorageRoots){if(!rootSupportsPresetSuffixes(root))continue;for(const sub of suffixes){targets.push({storage_path:root,games_path:joinScanPath(root,sub)});}}for(const appPath of buildInstalledTitleTargets()){targets.push({storage_path:inferStorageRoot(appPath),games_path:appPath});}for(const abs of (extras.absolute||[])){targets.push({storage_path:inferStorageRoot(abs),games_path:abs});}const dedupTargets=[];const seen={};for(const target of targets){const key=target.games_path||'';if(!key||seen[key])continue;seen[key]=1;dedupTargets.push(target);}const merged=[];let truncated=false;setGamesScanProgress(0,dedupTargets.length||1,'');for(let i=0;i<dedupTargets.length;i++){const target=dedupTargets[i];setGamesScanProgress(i,dedupTargets.length,target.games_path);const result=await scanGamesPath(target.games_path);if(!result.ok){if(result.status!==404&&typeof log==='function')log('Games scan skipped '+target.games_path+': '+result.error);setGamesScanProgress(i+1,dedupTargets.length,target.games_path);continue;}const rows=Array.isArray(result.data&&result.data.games)?result.data.games:[];for(const row of rows){const item=Object.assign({},row);item.games_path=target.games_path;item.storage_path=item.storage_path||target.storage_path||inferStorageRoot(item.path||'/');if(item.path)merged.push(item);}if(result.data&&result.data.truncated)truncated=true;setGamesScanProgress(i+1,dedupTargets.length,target.games_path);}const byPath={};for(const item of merged){const key=item.path||'';if(!key)continue;const prev=byPath[key];if(!prev||(!prev.has_cover&&item.has_cover)||(!prev.has_param_sfo&&item.has_param_sfo))byPath[key]=item;}gamesItems=Object.keys(byPath).map((k)=>byPath[k]);gamesItems.sort((a,b)=>String(a.path||'').localeCompare(String(b.path||'')));renderGamesStorageFilters();renderGames();let msg=String(gamesItems.length)+' '+tr('games_found_suffix');if(truncated)msg+=' '+tr('truncated');if(status)status.textContent=msg;setGamesScanProgress(dedupTargets.length,dedupTargets.length,'');}catch(err){if(status)status.textContent=tr('games_scan_failed')+': '+(err&&err.message?err.message:String(err));if(typeof log==='function')log('Games scan failed: '+(err&&err.message?err.message:String(err)));setGamesScanProgress(0,0,'');}finally{gamesScanBusy=false;if(scanBtn)scanBtn.disabled=false;}}"
                        "function bind(){const saved=(()=>{try{return localStorage.getItem(LANG_KEY)||localStorage.getItem('ps5drive_lang')||'';}catch(e){return '';}})();const nav=(navigator.language||'en');setLang(saved||nav);const sel=q('langSelect');if(sel)sel.addEventListener('change',()=>setLang(sel.value));const filesBtn=q('tabFilesBtn');const gamesBtn=q('tabGamesBtn');if(filesBtn)filesBtn.addEventListener('click',()=>setActiveTab('files'));if(gamesBtn)gamesBtn.addEventListener('click',()=>setActiveTab('games'));const scanBtn=q('gamesScanBtn');if(scanBtn)scanBtn.addEventListener('click',scanGames);const gamesPath=q('gamesPathInput');if(gamesPath){if(isPs4Build){gamesPath.value='';gamesPath.disabled=true;gamesPath.style.display='none';}else{gamesPath.addEventListener('keydown',(ev)=>{if(ev.key==='Enter'){ev.preventDefault();scanGames();}});}}wrap('updateSecurityMode',applyDynamicText);wrap('setPath',applyDynamicText);wrap('renderQueueInfo',applyDynamicText);wrap('renderRows',applyDynamicText);wrap('setListUIState',applyDynamicText);wrap('updateSelectionUI',applyDynamicText);setActiveTab('files');if(q('gamesStatus'))q('gamesStatus').textContent=tr('scan_ready');setGamesScanProgress(0,0,'');applyDynamicText();(async()=>{gamesStorageRoots=await fetchStorageRoots();syncVisibleStorage();renderGamesStorageFilters();renderGames();})();}"
                        "bind();"
                        "})();"
                        "</script></body></html>");
    if (rc != 0) {
